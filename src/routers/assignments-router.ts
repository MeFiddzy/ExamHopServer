...
assignmentsRouter.get('/classes/:classId/assignments', async (req: Request, res: Response) => {
...
assignmentsRouter.get('/assignments/:id', async (req: Request, res: Response) => {
...
assignmentsRouter.delete('/assignments/:id', async (req: Request, res: Response) => {
    const id = Number(req.params.id);
    const [assignment] = await db
        .select({ authorId: tables.assignments.authorId })
        .from(tables.assignments)
    ...
    await db.delete(tables.assignmentsToQuizzes).where(eq(tables.assignmentsToQuizzes.assignmentId, id));
    await db.delete(tables.assignments).where(eq(tables.assignments.id, id));
    res.status(200).json({ message: 'Assignment deleted' });
});

assignmentsRouter.get('/assignments/:id/quizzes', async (req: Request, res: Response) => {
    const id = Number(req.params.id);
    const rows = await db
        .select()
...
assignmentsRouter.delete('/assignments/:id/quizzes/:quizId', async (req: Request, res: Response) => {
    const id = Number(req.params.id);
